...
for tclass in classes:
    ...
    print("Class: " + tclass + " ---------------------------------")
    images = sorted(glob.glob(os.path.join(class_path, '*.*[gG]')))
    total_image_count += len(images)
    count = 1
    for image in images:
        # 画像チェック
        if imghdr.what(image) is None:
            print("Error: Not image " + image)
            continue

        # 集めた画像データから顔が検知されたら、切り取り、保存する。
        print("["+str(count)+"/"+str(len(images))+"] " + image)
        ...
        if FLAGS.method is "wrap":
            ...
            # 画像の切り出し
            processed_img = org_img[top:bottom, left:right]
